...
- Gate the five-layer structure on `slot !== 'marker'`. For `marker`, render just: a wrapper at 34px, one dot `inset-1/3 rounded-full bg-accent` with `boxShadow: '0 0 14px var(--accent)'`, `className={pipelineLive ? 'animate-recpulse' : ''}`.
